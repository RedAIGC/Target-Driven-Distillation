...
import random
from pcm_discriminator_flux import TransformerFluxDiscriminator
from pcm_scheduling_flowmatch_modified import FlowMatchEulerDiscreteScheduler
...
def log_validation(
    ...
    for tracker in accelerator.trackers:
        ...
        if tracker.name == "tensorboard":
            ...
            tracker.writer.add_images(f"{idx}:"+pipeline_args["prompt"], np_images, step, dataformats="NHWC")
            if save_path:
                img = Image.fromarray(np_images[0].astype(np.uint8))
                img.save(save_path)
...
def main(args):
    ...
    discriminator = TransformerFluxDiscriminator(transformer)
    discriminator.transformer.requires_grad_(False)
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            ...
            for model in models:
                print(f"Model type: {model.__class__}")
            ...
            for model in models:
                if isinstance(model, type(unwrap_model(transformer))):
                    ...
                    print("Not Saving TransformerFluxDiscriminator model...")
    ...
    transformer, discriminator, optimizer, discriminator_optimizer, lr_scheduler = accelerator.prepare(transformer, discriminator, optimizer, discriminator_optimizer, lr_scheduler)
    ...
    global_step = 0
    first_epoch = 0
    # 假设使用 float32 类型 (每个元素占 4 字节)
    total_memory = 0  # 总内存占用（字节）

    # 遍历每个参数，计算它的内存占用
    for param in transformer_lora_parameters:
        param_size_bytes = param.numel() * param.element_size()  # 计算每个参数的内存占用
        total_memory += param_size_bytes
    # 将字节转换为 MB
    total_memory_mb = total_memory / (1024 ** 2)  # 1 MB = 1024 * 1024 字节
    print(f"Total memory used by transformer_lora_parameters: {total_memory_mb:.2f} MB")
    ...
    # 将字节转换为 MB
    total_memory_mb = total_memory / (1024 ** 2)  # 1 MB = 1024 * 1024 字节
    print(f"Total memory used by discriminator_lora_parameters: {total_memory_mb:.2f} MB")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                if args.guidance_scale == "random_0_3":
                    # 在0到3之间随机选择一个浮动的值
                    guidance_value = random.uniform(0, 3)
                else:
                    # 如果不是"random_0_3"，则使用args.guidance_scale的实际值
                    guidance_value = float(args.guidance_scale)

                # 转换为torch.tensor并移动到正确的设备
                guidance = torch.tensor([guidance_value], device=accelerator.device)
                ...
                gan_timesteps = torch.empty_like(timestep_index_s)
                for i in range(timestep_index_s.size(0)):
                    gan_timesteps[i] = torch.randint(
                        timestep_index_s[i].item(),
                        min(timestep_index_s[i].item() + noise_scheduler.config.num_train_timesteps // args.num_inference_steps_min,
                            noise_scheduler.config.num_train_timesteps),
                        (1,),
                        dtype=timestep_index_s.dtype,
                        device=timestep_index_s.device,
                    )
                real_gan = noise_scheduler.noise_travel(
                    target, torch.randn_like(target), timestep_index_s, gan_timesteps
                )
                fake_gan = noise_scheduler.noise_travel(
                    model_pred, torch.randn_like(model_pred), timestep_index_s, gan_timesteps
                )
                ...
                if global_step % 2 == 0:
                    ...
                    loss = discriminator(
                        ...
                        prompt_embeds.float(),
                        text_ids.float(),
                        latent_image_ids.float(), # 注意：不需要prepared
                        1,
                    )
                    loss+=zero_sum
                    ...
                    if accelerator.sync_gradients:
                        accelerator.clip_grad_norm_(
                            discriminator_params, args.max_grad_norm
                        )
            ...
            # for param in transformer_lora_parameters[:5]:
            #     print(f" requires_grad: {param.requires_grad}")
            # for param in discriminator_params[:5]:
            #     print(f"requires_grad: {param.requires_grad}")
            ...
            if accelerator.is_main_process:
                if global_step==1 or  global_step % args.validation_steps == 0:
                    ...
                    validation_prompts = [
                        ...
                        "In the bustling market square, a baker displays an array of freshly baked pastries, the aroma enticing passersby to stop and indulge.",
                        "Beneath a starry sky, an astronomer peers through a telescope from a grassy hilltop, searching the cosmos for celestial wonders.",
                        "In a park at sunset, an elderly man with glasses smiles warmly at his grandchildren, his wrinkled face radiating love and contentment.",
                        "In a bustling city, a woman in a vibrant scarf stands at a crosswalk, her face framed by the city lights as she looks around curiously.",
                        "In a quiet library, a student with focused eyes types on a laptop, the soft light casting a gentle glow on their thoughtful expression.",
                        "At a wedding reception, a groom gazes lovingly at his bride, his face full of emotion as he smiles with joy and excitement.",
                        "In a busy subway station, a young man with headphones on his ears taps his foot to the beat, a relaxed and content expression on his face.",
                    ...
                    for prompt in validation_prompts:
                        ...
                        save_path=os.path.join(samples_dir,f"{i:06d}_{global_step:06d}.jpg")
                        with torch.no_grad():
