...
from pcm_scheduling_ddpm_modified import DDPMScheduler
...
@torch.no_grad()
def log_validation(
    vae, unet, text_encoder_one, text_encoder_two, tokenizer_one, tokenizer_two, args, accelerator, weight_dtype, step, cfg, num_inference_step
):
    ...
    validation_prompts = [
        ...
        "A young woman stands at the edge of a dense forest, her long hair flowing in the breeze. In the distance, sunlight filters through the leaves, casting dappled shadows on the moss-covered ground.",
        "On a grassy plain, a shepherd wearing a worn cloak gazes towards distant mountains. Beside him, his flock of sheep grazes leisurely, while a few white clouds drift across the distant sky.",
        "In front of a small cabin by the lake, an elderly fisherman prepares his boat. The surface of the lake shimmers, and a white swan glides across, leaving behind a beautiful ripple.",
        "Along a rugged mountain path, a young hiker with a backpack strides towards the summit. Surrounding him are dense pine forests and distant peaks.",
        "In the courtyard of an ancient castle, a young couple strolls hand in hand. Vines cling to the ancient stone pillars, and fluffy white clouds drift across the sky.",
        "In a golden wheat field, a farmer swings a sickle to harvest the crops. A small yellow sparrow perches on his shoulder, as if engaged in conversation.",
        "By the river in a small town, a group of children frolics in the clear water. Willow trees sway gently on the bank, while a light breeze carries the scent of freshness.",
        "Deep in the heart of a tropical rainforest, a cheetah prowls silently among the dense foliage. Sunlight filters through the leaves, creating dappled patterns on the forest floor.",
        "Amidst ancient ruins, an archaeologist excavates buried treasures. Carved into the ancient pillars are mysterious symbols, seeming to recount tales of history.",
        "At the summit of a snow-capped mountain, a young mountaineer stands, admiring the breathtaking vista. Snowflakes dance in the air, and the distant valley echoes with the chirping of birds.",
    ]
    ...
    for idx, prompt in enumerate(validation_prompts):
        ...
        image_logs.append({"validation_prompt": prompt, "images": images, "idx": idx})
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            for log in image_logs:
                ...
                total_width = sum(img.width for img in formatted_images)
                max_height = max(img.height for img in formatted_images)
                ...
                target_output_dir = os.path.join(args.output_dir, "samples")
                os.makedirs(target_output_dir, exist_ok=True)
                
                combined_image.save(os.path.join(target_output_dir, f"{cfg}_{idx:05d}_{step:06d}.png"))
                combined_np_img = np.array(combined_image)
                combined_np_img = np.expand_dims(combined_np_img, axis=0)  # Add batch dimension
                tracker.writer.add_images(
                    f"{cfg}: " + validation_prompt, combined_np_img, step, dataformats="NHWC"
                )
...
class DDIMSolver:
    def __init__(self, alpha_cumprods, timesteps=1000, ddim_timesteps=50,
        ...
        # tdd parameters(DDPM)
        self.tdd_alpha_cumprods = alpha_cumprods
        ...
        # tdd parameters(DDPM)
        self.tdd_alpha_cumprods = torch.from_numpy(self.tdd_alpha_cumprods)
    ...
    def to(self, device):
        ...
        self.tdd_alpha_cumprods = self.tdd_alpha_cumprods.to(device)
    ...
    def set_timesteps_s(
        ...
        merged_timesteps = []
        origin_timesteps = self.ddim_timesteps[::-1].copy()
    ...
    def tdd_step(self, pred_x0, pred_noise, timestep_index_s):
        alpha_cumprod_s = extract_into_tensor(self.tdd_alpha_cumprods, timestep_index_s, pred_x0.shape)
        dir_xt = (1.0 - alpha_cumprod_s).sqrt() * pred_noise
        x_prev_s = alpha_cumprod_s.sqrt() * pred_x0 + dir_xt
        return x_prev_s
...
def main(args):
    ...
    solver = DDIMSolver(
        ...
        num_inference_steps_min=args.num_inference_steps_min,
        num_inference_steps_max=args.num_inference_steps_max,
    )
    ...
    unet, discriminator, optimizer, discriminator_optimizer, lr_scheduler = accelerator.prepare(unet, discriminator, optimizer, discriminator_optimizer, lr_scheduler)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                timesteps_s = solver.select_s(timesteps, args.s_ratio)
                model_pred = solver.tdd_step(pred_x_0, noise_pred, timesteps_s)
                ...
                gan_timesteps = torch.empty_like(timesteps_s)
                for i in range(timesteps_s.size(0)):
                    gan_timesteps[i] = torch.randint(
                        timesteps_s[i].item(),
                        min(timesteps_s[i].item() + noise_scheduler.config.num_train_timesteps // args.num_inference_steps_min,
                            noise_scheduler.config.num_train_timesteps),
                        (1,),
                        dtype=timesteps_s.dtype,
                        device=timesteps_s.device,
                    )
                real_gan = noise_scheduler.noise_travel(
                    target, torch.randn_like(latents), timesteps_s, gan_timesteps
                )
                fake_gan = noise_scheduler.noise_travel(
                    model_pred, torch.randn_like(latents), timesteps_s, gan_timesteps
                )
